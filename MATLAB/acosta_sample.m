...
tt=0:T/2:Tu;
figure(1);
subplot(121); stem(tt(1:20),real(carriers(1:20))); hold on; stem(tt(1:20),imag(carriers(1:20)),'g'); hold off;
f=(2/T)*(1:(FS))/(FS);
subplot(222); plot(f,abs(fft(carriers,FS))/FS); 
subplot(224); pwelch(carriers,[],[],[],2/T, 'center');
...
figure(2);
subplot(221); stem(p,g);
dummy=conv(g,chips(:));
u=[dummy(1:length(t))]; % (C)
% u=[dummy; zeros(46,1)];

subplot(223); plot(t(1:400),real(u(1:400))); hold on; plot(t(1:400),imag(u(1:400)),'g'); hold off;
ff=(Rs)*(1:(q*FS))/(q*FS);
subplot(222); plot(ff,abs(fft(u,q*FS))/FS);
subplot(224); pwelch(u,[],[],[],Rs, 'center');
...
subplot(223); plot(t(80:480),real(uoft(80:480))); hold on; plot(t(80:480),imag(uoft(80:480)),'g' ); hold off;
subplot(222);
plot(ff,abs(fft(uoft,q*FS))/FS);
subplot(224);
pwelch(uoft,[],[],[],Rs, 'center');
...
figure(4);
subplot(221); plot(t(80:480),s(80:480)); 
subplot(223); plot(t(80:480),imag(s_tilde(80:480)),'g');
subplot(222);
...
% figure(1);
% subplot(211); plot(t,real(r_tilde));
% axis([0e-7 12e-7 -60 60]);
% grid on;
% figure(1);
% subplot(212); plot(t,imag(r_tilde));
% axis([0e-7 12e-7 -100 150]);
% grid on;
% figure(2); 
% ff=(Rs)*(1:(q*FS))/(q*FS); 
% subplot(211); plot(ff,abs(fft(r_tilde,q*FS))/FS); grid on;
% figure(2);
% subplot(212); pwelch(r_tilde,[],[],[],Rs);
% 
% %Carrier suppression
% 
% [B,AA] = butter(3,1/2);
...
% figure(6);
% f=(2/T)*(1:(FS))/(FS);
% subplot(211); plot(f,abs(fft(r_data,FS))/FS);
% grid on;
% subplot(212); pwelch(r_data,[],[],[],2/T);
% 
% %FFT
