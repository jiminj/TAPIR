...
namespace Tapir {
    ...
    void MatrixInterleaver::interleave(const DSPSplitComplex * src, const DSPSplitComplex * dest) const
    {
        vDSP_mtrans(src->realp, 1, dest->realp, 1, m_nCols, m_nRows);
        vDSP_mtrans(src->imagp, 1, dest->imagp, 1, m_nCols, m_nRows);
    }
    void MatrixInterleaver::deinterleave(const DSPSplitComplex * src, const DSPSplitComplex * dest) const
    {
